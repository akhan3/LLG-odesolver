function [Mnext] = odeStepMat(sp, bc, M, Hext)
    if sp.odeSolver_rk4
...
function [H] = Hfield(sp, bc, M, Hext)
    ...
    H(:,1  ,:) = sp.cCoupl * bc.Mtop(:,:);   % top row
    H(:,end,:) = sp.cCoupl * bc.Mbot(:,:);   % bottom row
    H(:,:,end) = sp.cCoupl * bc.Mrig(:,:);   % right column
    H(:,:  ,1) = sp.cCoupl * bc.Mlef(:,:);   % left column
